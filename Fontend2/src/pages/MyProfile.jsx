...
const MyProfile = () => {
  ...
  const getProfile = async () => {
    try {
      const id = parseInt(user?.nameid, 10);
      if (isNaN(id) || id <= 0) {
  ...
  }, [token, user?.nameid, loading]);

  return token ? (
    <div className="flex flex-col gap-2 text-sm justify-center items-center ">
      <div className="border border-gray-300 w-[50%] items-center flex flex-col justify-center">
        {error && (
        ...
        <img
          ...
          src={
            userData?.image || "https://www.w3schools.com/w3images/avatar2.png"
          }
          alt=""
        />
        {isEdit ? (
